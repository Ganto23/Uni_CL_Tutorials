...
sudokuProblem = And [ Or [P (1,8,8)], Or [P (1,9,2)], Or [P (2,1,6)]
                    ...
                    , Or [P (1,1,1)], Or [P (1,2,3)], Or [P (1,3,4)]
                    , Or [P (1,4,7)], Or [P (1,6,6)], Or [P (1,7,5)]
                    , Or [P (2,2,5)], Or [P (2,3,7)]]
...
The following constraints give a minimal and complete description of the Sudoku game:
allFilled - Checks that every cell in the puzzle is filled
noneFilledTwice - Checks that every cell is not filled twice
rowsComplete - Checks that each has all digits 1-9
columnsComplete - Checks that each column has all digits 1-9
squaresComplete - Checks that each subsquare has all digits 1-9

Therefore the following are redundant:
rowsNoRepetition - If rowsComplete is true then this must be true since there are only 9 spaces
columnsNoRepetition - If columnsComplete is true then this must be true since there are only 9 spaces
squaresNoRepetition - If squaresComplete is true then this must be true since there are only 9 spaces


We would not improve efficiency of the solver by removing redundant constraints since they are used to 
restrict the search space of the dpll algorithm by reducung number of false avenues. The effect of this
is greater than the computation time required for the extra clauses to be computed.



Call rowsNoRepetition = r
Call columnsNoRepetition = c
Call squaresNoRepetition = s

Order of fastest Computations (based on printSolution . head . solutions $ sudokuProblem)
All used - 6.07s
Excluding s - 
Excluding c - 158s
...
Runs and compiles but doesnt actually give an answer
squaresNoRepetition = And [ Or [ N (i, j, n), N (i', j', n) ]
                        | n <- [1..9],
                          a <- [0..2], i <- [(3*a + 1)..(3*a + 3)], i' <- [(3*a + 1)..(i-1)],
                          b <- [0..2], j <- [(3*b + 1)..(3*b + 3)], j' <- [(3*b + 1)..(j-1)] ]


Also runs and compiles take wayyyyy too long
squaresNoRepetition = And [ Or [ N (i, j, n), N (i', j', n)]
                        | a <- [0..2], b <- [0..2], n <- [1..9],
                          i <- [(3*a+1)..(3*a+3)], i' <- [(3*a+1)..(i-1)],
                          j <- [(3*b+1)..(3*b+3)], j' <- [(3*b+1)..(j-1)] ]
-}
